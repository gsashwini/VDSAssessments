...
public class Parser {
	...
	public static void main(String[] args) {
		...
		FileWriter file;
		try {
			file = new FileWriter(System.getProperty("user.dir") + "\\ResultOfJson\\assessments.json");
	...
	public static void createJSON(String fileName, File[] paths, int count) {
		...
		for (int j = 0; j < nList.getLength(); j++) {
			...
			else if(element.getElementsByTagName("matflashparam").item(0) != null) {
				...
				// Directory where the files are located
					...
					for (int i = 0; i < pathD.length; i++) {
						...
						if(file.equals(uri)) {
							...
							System.out.println("Question "+DandDQues);

							questionsList.add(DandDQues);
							String DandDAns = parseDandDAns(file, pathD,i);
							System.out.println("Answers "+DandDAns);
			...
			//System.out.println("FIB question1");
			...
			//	if(!questionsList.get(0).toString().contains("In your opinion")) {
				{
					...
					//System.out.println("FIB question2");
					...
					if(type.equalsIgnoreCase("FIB")) {
						...
						for(int i=0; i<element.getElementsByTagName("mattext").getLength(); i++) {
							...
							//System.out.println("FIB Questions " +element.getElementsByTagName("mattext").item(i).getTextContent());
	...
	public static String parseDandDAns(String fileName, File paths[],int da) {
		DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
		DocumentBuilder dBuilder = null;
		Document doc = null;
